...
def get_features(trials):
    ...
    features = ('target','sex',
                ...
                'wpowerx','wpowery','wpowerz','wpower')+ \
                tuple(['fx'+str(i) for i in range(wstop)])+ \
                tuple(['fy'+str(i) for i in range(wstop)])+ \
                tuple(['fz'+str(i) for i in range(wstop)])+ \
                tuple(['f'+str(i) for i in range(wstop)]) + \
                tuple(['vx'+str(i) for i in range(wstop)])+ \
                tuple(['vy'+str(i) for i in range(wstop)])+ \
                tuple(['vz'+str(i) for i in range(wstop)])+ \
                tuple(['v'+str(i) for i in range(wstop)])
    ...
    for i,trial in enumerate(trials):
        ...
        tmp['acx'],tmp['acy'],tmp['acz'] = [ac(data[:,j],10) for j in range(3)]
        tmp['corrxy'],tmp['corrxz'],tmp['corryz'] = [cc(data[:,j],data[:,k],0) for j,k in [(0,1),(0,2),(1,2)]]
        w,fft = myfft(data)
        w1d,fft1d = myfft(data1d)
        fft = [np.interp(winterp,w,med1d(fft[:,j],5))for j in range(3)]
        fft1d = np.interp(winterp,w1d,med1d(fft1d,5))
        stft = [np.abs(librosa.core.stft(data[:,j],n_fft=256,hop_length=2,win_length=32)) for j in range(3)]
        stft1d = np.abs(librosa.core.stft(data1d,n_fft=256,hop_length=2,win_length=32))
        varstft = [np.interp(winterp,w,med1d(np.var(stft[j],axis=0),5)) for j in range(3)]
        varstft1d = np.interp(winterp,w,med1d(np.var(stft1d,axis=0),5))
        tmp['sigwx'],tmp['sigwy'],tmp['sigwz'] = [sigw(winterp,fft[j]) for j in range(3)]
        tmp['sigw'] = sigw(winterp,fft1d)
        tmp['wpowerx'],tmp['wpowery'],tmp['wpowerz'] = [np.sum(fft[j]) for j in range(3)]
        tmp['wpower'] = np.sum(fft1d)
        fftbins = [fft[j].reshape((wstop,1./wstep)).mean(axis=1) for j in range(3)]
        fftbins1d = fft1d.reshape((wstop,1./wstep)).mean(axis=1)
        fftbins = [fftbins[j]/fftbins[j].sum() for j in range(3)]
        fftbins1d = fftbins1d/fftbins1d.sum()
        vbins = [varstft[j].reshape((wstop,1./wstep)).mean(axis=1) for j in range(3)]
        vbins1d = varstft1d.reshape((wstop,1./wstep)).mean(axis=1)
        for k in range(len(vbins1d)):
            tmp['fx'+str(k)],tmp['fy'+str(k)],tmp['fz'+str(k)] = [fftbins[j][k] for j in range(3)]
            tmp['f'+str(k)] = fftbins1d[k]
            tmp['vx'+str(k)],tmp['vy'+str(k)],tmp['vz'+str(k)] = [vbins[j][k] for j in range(3)]
            tmp['v'+str(k)] = vbins1d[k]
        df.loc[i] = tmp
...
def main():
    ...
    #clf = xgboost, or something else
    fitparam = {'colsample_by_tree': 1,'eta': 0.3,'eval_metric': 'mlogloss','gamma': 0,
         'max_delta_step': 0,'max_depth': 3,'min_child_weight': 1,'num_class': 12,
         'num_round': 300,'objective': 'multi:softprob','silent': 1,'subsample': 1}
    clf = GradientBoostingClassifier(fitparam)
